...
- **Source**: CSV file (`Bayview.YYYYMMDD.csv.gz`) - gzipped CSV
- **Update Frequency**: Monthly
- **Key Fields**:
  - `"E-Mail"` (varchar): Instructor's email address
  - `"Instructor"` (varchar): Instructor's name
  - `"FirstName"` (varchar): Instructor's first name
  - `"LastName"` (varchar): Instructor's last name
...
- **Key Fields**:
  ...
  - `sector` (integer): Institution sector
  - `iclevel` (integer): Institution level
  - `control` (integer): Control type
  - `instsize` (integer): Institution size category
...
- **Fields**: All survey fields plus:
  - IPEDS institutional information (instnm, sector, iclevel, control, instsize, etc.)
  - Opt-out status (`is_opted_out`)
  - Opt-out source (`opt_out_source`)
  - Standardized period format (`period_sortable`) in YYYY-N format where N is 1-4 for Winter, Spring, Summer, Fall
...
- **Filtering Logic**:
  ...
  - Email must not be in opt-out list
...
- **Filtering Logic**: Uses a recent_periods view to identify the 8 most recent periods

#### State-Specific Mailing Lists
Four views for state-specific mailing lists:
- **california_mailing**: Records from California (State = 'CA')
- **texas_mailing**: Records from Texas (State = 'TX')
- **florida_mailing**: Records from Florida (State = 'FL')
- **newyork_mailing**: Records from New York (State = 'NY')
- **Generated By**: `1_mailing_lists.sql`
...
- **Key Fields**:
  ...
  - `"State"` (varchar): State
  - `record_counts_by_period` (list): Count of records by period in format "YYYY-N:count"
  - `section_counts_by_period` (list): Count of unique sections by period in format "YYYY-N:count"
...
- **Key Fields**:
  ...
  - `publishers` (list): List of distinct publishers used
...
- **Key Fields**:
  ...
  - `"School"` (varchar): Institution
  - `period_sortable` (varchar): Standardized period
  - `sections_in_period` (integer): Count of distinct sections
