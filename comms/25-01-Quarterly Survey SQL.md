...
!Work/Commodore/media/Opt_out.csv
!Work/Commodore/media/hdic*dist*2022_selected.csv
## Description
...
> [!info] ℹ️
> Nota Bene
The following SQL uses `JOIN`s to create new tables. It would also be possible to use `ALTER TABLE` and `UPDATE` to modify/combine table content in-place, which could be more space efficient.
...
```
## 3.5 Other Enrichment 
TBD, likely includes cost
## 4. Subset for latest periods
> Create a new database with a subset of records selected from the master database using the latest `n = 8` periods:
...
WHERE SortablePeriod IN ('2024-2', '2024-1', '2023-4', '2023-3')
  ...
  AND OptOut IS NULL;
```
For the latest four period prior to a reference *period `ref*period`:
```sql
...
```
# Tooling
Suggestions and commentary courtesy of ChatGPT
## DBViewer
...
8. **Create Relationships and Perform Operations**
    - **Create Relationships**:
...
9. **Add a New Sortable Period Column**
...
11. **Create a New Database with a Subset of Records Based on Reference Period**
