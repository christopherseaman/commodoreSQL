...
flowchart TD
  ...
  i0c --> i00["02 · 0_setup.sql"] --> i0b["03 · 0b_state_region.sql"] --> i10["04 · 1_bookprices_import.sql"] --> i1a["05 · 1a_supply_classification.sql"] --> i1b["06 · 1b_section_enrollment.sql"] --> i20["07 · 2_oer_classification.sql"] --> i2b["08 · 2b_course_material.sql"] --> i2c["09 · 2c_pricing_wide.sql"] --> i2d["10 · 2d_data_quality.sql"]
  i2d --> e30["11 · 3_mailing_lists.sql"] --> e3b["12 · 3b_master_material.sql"] --> e40["13 · 4_merged_records.sql"] --> m01["14 · models/master_institution.sql"] --> m02["15 · models/master_isbn.sql"] --> m03["16 · models/sample_material_10pct.sql"] --> x01
  i2d -. "NO_EDA" .-> x01
...
| `course_material_post_2024` | Post-2024 projection. |
| `course_material_use` | Use projection. |
| `course_material_no_use` | Exact post-2024 complement. |
...
Solid arrows are dependencies; dotted arrows are projections/subsets/leaves. All 33 DBML relations are consumed by the staged SQL. Geographic mailing is seven direct `current_mailing` export filters; Canada material exports filter `course_material_no_use` directly.
...
flowchart TD
  ...
  course_material --> course_material_post_2024
  course_material_post_2024 --> course_material_use
  course_material_post_2024 --> course_material_no_use
...
`state_region` is an IMPORT lookup joined at query time. The intended topology is 33 DBML relations (26 tables, seven views); cleanup removes retired names. `section_enrollment` uses IPEDS for its reference population and fallback medians. Requiredness is computed within `comprehensive_data`.
