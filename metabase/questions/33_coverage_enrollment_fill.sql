...
FROM (
    ...
    UNION ALL SELECT 3, 'missing w/ sibling enrollment',        COUNT(*) FILTER (WHERE NOT has_enrollment AND enrollment_has_sibling)       FROM master_section
    UNION ALL SELECT 4, 'missing w/ own seats_taken',           COUNT(*) FILTER (WHERE NOT has_enrollment AND enrollment_has_own_seats)     FROM master_section
    UNION ALL SELECT 5, 'missing w/ sibling seats_taken',       COUNT(*) FILTER (WHERE NOT has_enrollment AND enrollment_has_sibling_seats) FROM master_section
    UNION ALL SELECT 6, 'unfillable (no signal)',               COUNT(*) FILTER (WHERE NOT has_enrollment
                                                                   AND NOT enrollment_has_sibling
                                                                   AND NOT enrollment_has_own_seats
                                                                   AND NOT enrollment_has_sibling_seats)                FROM master_section
) ORDER BY ord
