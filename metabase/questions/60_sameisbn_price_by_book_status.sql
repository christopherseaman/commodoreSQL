...
-- display: table
-- description: Issue #44 robustness cut: uses the literal book_status field from canonical Use rows in comprehensive_data instead of is_required_inferred. required = book_status='required'; supplemental = book_status IN ('option','recommended'). Excludes (section,isbn) materials carrying both statuses (mixed/ambiguous). Same demeaning method as the primary same-ISBN question: subtract each ISBN's own overall median price_min, requiring >=10 total adoptions and at least 1 in each status. FINDING: essentially no premium survives this stricter literal cut — demeaned mean +$0.52 (required) vs +$0.35 (option/recommended), a ~$0.17 gap (median residual $0.00 for both) — versus a raw mean gap of ~$1.5 ($77.90 vs $76.38). The residual is a rounding-scale fraction of the ~$60 median price, plausibly channel-mix (required adoptions carry a rental option more often) rather than price discrimination. Read with the primary is_required_inferred question: no economically meaningful same-item required-vs-optional pricing effect at BMG scope. price_min = cheapest option (buy/rent, any condition/format) from pricing_wide.
WITH scope AS (
  SELECT section_id
  FROM master_section
