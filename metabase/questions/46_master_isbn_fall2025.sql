...
-- display: table
-- description: BMG task #37. One record per unique (ISBN13, Title, Author, Format, FormatType) across ALL Fall-2025 (period 2025-4) catalog rows — built to reveal how the same/similar material is listed multiple ways so the team can decide whether to combine listings. Missing-ISBN rows are INCLUDED as their own combos (blank source cells import as NULL, ~54% of catalog). Counts per combo: NumReq/NumOpt/NumRec/NumBlnk by explicit book_status; NumBVAReq = BVA logic (is_required_inferred, inferred-required #1); NumTot = all rows. NumReq+NumOpt+NumRec+NumBlnk = NumTot by construction. First-non-blank Publisher & Imprint (from catalog) and Edition (from pricing_historical, per-ISBN; ~67% of priced ISBNs, blank for missing-ISBN combos). PublishedYear is not present in the source data (omitted). IsSupply flags ISBNs the #36 title-keyword classifier identifies as bookstore supplies (lab kits, goggles, calculators, ...) — surfaced, NOT excluded, so the team can decide whether to drop them when combining listings. This is an intentionally broad raw-listing audit, not a canonical #58 Use release model. ~348,826 records; Metabase shows 2,000, full set exportable. Ordered by NumTot desc (most-listed first); re-sort by ISBN13 to cluster listing variants.
WITH ed AS (
