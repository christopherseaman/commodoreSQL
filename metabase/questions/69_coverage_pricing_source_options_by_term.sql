-- name: Coverage & Scope — Source Pricing Options by Term
-- display: table
-- description: All deduplicated source-pricing observations by term/option/condition/format; no Use/required filter. Latest-snapshot source coverage at the BMG-owned pricing_historical observation grain, grouped by term × source option × condition × format with no canonical Use, required, or inferred-required filter. valid_price means price < 9999, so zero is valid; key counts use distinct source section × ISBN pairs. This source denominator is deliberately different from canonical master_material items.
