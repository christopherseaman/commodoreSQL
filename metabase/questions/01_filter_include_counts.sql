...
-- display: table
-- description: Raw recent-term catalog rows, by inferred-required flag, level, and term. Legacy raw diagnostic of the inferred-required flag, grouped by course level and recent term. The FALSE group means only "not inferred required": it also retains literal-required rows that fail the section-level inference rule, NULL statuses, supplies, placeholders, Canada, and other NoUse rows. This is not the canonical Use population or a release metric.
