...
| #36 | Supply-vs-course-material ISBN classifier | `scripts/sql/lookups/supply_keywords.tsv` (84 incl + 26 excl), `scripts/classify_supplies.sh` |
...
"Required" = **`filter_include`** (inferred is_required, #1). Magnitudes: A=2,525,891 / B=127,270.

### Held / pending decisions (do NOT start without sign-off)

- **#39 dashboards + self-serve** — HELD after proposal. Decided: **Metabase Models + GUI cards**,
  a **themed set of 3** boards (Overview / Cost-hypothesis / Enrollment-DQ). User is reviewing the
  detailed proposal before build. See the "Proposed dashboard" + this section.
- **#32** — persist `enrollment_assigned` as a **column on `master_section`** (currently
  analysis-layer only, in questions 44/45). Method confirmed; persistence needs the go-ahead.
- **#36 integration** — persist `is_supply` as a **column on `master_section`** and regenerate the
  subsets/tallies excluding supplies. Keyword list needs review first.
- **Hypothesis test** (enrollment-weighted / same-item cost across classes) — HELD; documented as
  a proposal in Notion. Blocked on #32/#36 landing first.
